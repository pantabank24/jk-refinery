...
// How long a locked price stands, with the API's default. The page saves every
...
const SELL_LOCK_DEFAULTS: Record<string, string> = {
  sell_price_lock_seconds: "10",
};
...
export default function SalesPricePage() {
  ...
  const fetchAll = async () => {
    try {
      ...
      const map: Record<string, string> = { ...SELL_LOCK_DEFAULTS };
    ...
    try {
      await Promise.all(
        ["sales_enabled", "sales_open_time", "sales_close_time", "sales_realtime_after_hours", "sales_realtime_until", "sales_realtime_only",
          "realtime_premium_thb", "realtime_spread_thb", ...Object.keys(SELL_LOCK_DEFAULTS)]
      ...
      <div className="flex flex-col gap-y-4 md:overflow-y-auto pb-6">
        ...
        {masterOn && (
          <>
            ...
            {/* The window the shop stands behind a quoted price for. */}
            <div className="flex flex-col border-1 border-black/10 bg-black/5 backdrop-blur-xl rounded-3xl p-5 gap-y-4">
              <div className="flex flex-col">
                <span className="font-bold text-md flex items-center gap-x-2">
                  <ShieldCheck size={16} className="text-[#c09c42]" /> เวลาล็อกราคา (ลูกค้ากดขายเอง)
                </span>
                <span className="text-xs text-black/50">
                  ตอนลูกค้ากดส่งขาย ระบบจะคิดราคาจากราคาตลาดของร้านเองแล้วล็อกไว้ให้ตามเวลานี้
                  ลูกค้าเห็นราคาที่ล็อกในหน้ายืนยัน และได้ราคานั้นตรงทุกบาทถ้ากดยืนยันทัน
                </span>
              </div>

              <Input
                type="number" min={3} max={60} label="ล็อกราคานาน (วินาที)"
                value={cfg["sell_price_lock_seconds"] ?? ""}
                isDisabled={!canEdit} onValueChange={(v) => setC("sell_price_lock_seconds", v)}
                description="ตั้งได้ 3–60 วินาที แนะนำ 10"
                classNames={{ inputWrapper: "bg-gradient-to-br from-black/10 to-transparent border-1 border-black/10 rounded-2xl" }}
              />

              <div className="text-xs text-black/60 bg-black/5 rounded-2xl px-4 py-3 leading-relaxed">
                ยิ่งล็อกนาน ลูกค้ายิ่งมีเวลาอ่านและกดยืนยัน แต่ถ้าตลาดขยับแรง
                ร้านต้องซื้อที่ราคาที่ล็อกไว้แม้ราคาจะตกไปแล้ว
                · ใช้กับลูกค้ากดขายเองทั้งทองและเงิน พนักงานที่ขายแทนลูกค้ายังใส่ราคาเองได้
              </div>
            </div>
