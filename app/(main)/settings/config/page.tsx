...
import { Save, ChevronRight, Tag, Keyboard, Coins, Zap, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="flex flex-col md:h-full gap-y-4">
      ...
      <div className="flex flex-col gap-y-4 md:overflow-y-auto pb-4">
        ...
        {/* PDPA consent — opens a dedicated page */}
        <Link
          href="/settings/pdpa"
          className="flex flex-row items-center justify-between border-1 border-black/10 bg-black/5 hover:bg-black/10 transition-colors backdrop-blur-xl rounded-3xl p-5"
        >
          <div className="flex flex-row items-center gap-x-3">
            <span className="text-[#c09c42]"><ShieldCheck size={20} /></span>
            <div className="flex flex-col">
              <span className="font-bold text-md bg-gradient-to-l from-black/90 to-yellow-600 bg-clip-text text-transparent">
                ประกาศความเป็นส่วนตัว (PDPA)
              </span>
              <span className="text-xs text-black/50">
                ประกาศที่ลูกค้าต้องกดรับทราบตอนเข้าระบบ · เวอร์ชัน · ความยินยอมรับข่าวสาร
              </span>
            </div>
          </div>
          <ChevronRight size={20} className="text-black/40" />
